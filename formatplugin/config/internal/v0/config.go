...
func upgradeAssets(cfg *Config, factory formatplugin.Factory) (changed bool, rErr error) {
	...
	for _, k := range sortedKeys {
		...
		if err != nil {
			return false, errors.Wrapf(err, "failed to marshal asset configuration for formatter %q as YAML", k)
		}

		upgradedBytes, err := upgrader.UpgradeConfig(assetCfgBytes)
		if err != nil {
			return false, errors.Wrapf(err, "failed to upgrade configuration for formatter %q", k)
		}
		...
		if err := yaml.Unmarshal(upgradedBytes, &yamlRep); err != nil {
			return false, errors.Wrapf(err, "failed to unmarshal YAML of upgraded configuration for formatter %q", k)
		}
